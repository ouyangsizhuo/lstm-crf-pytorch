...
def load_data():
    data = []
    if KEEP_IDX:
        cti = load_tkn_to_idx("./prepare_data/train.txt" + ".char_to_idx")
        wti = load_tkn_to_idx("./prepare_data/train.txt" + ".word_to_idx")
        tti = load_tkn_to_idx("./prepare_data/train.txt" + ".tag_to_idx")
    else:
    ...
    fo = open("./prepare_data/train.txt")
    if HRE:
        tmp = []
...
if __name__ == "__main__":
    ...
    save_data("./prepare_data/train.txt" + ".csv", data)
    if not KEEP_IDX:
        save_tkn_to_idx("./prepare_data/train.txt" + ".char_to_idx", cti)
        save_tkn_to_idx("./prepare_data/train.txt" + ".word_to_idx", wti)
        save_tkn_to_idx("./prepare_data/train.txt" + ".tag_to_idx", tti)
